          ...
          return (
            <Card key={unit.id} className="cursor-pointer hover:ring-1 hover:ring-primary/30 transition-all" onClick={() => navigate(`/app/records/inventory/${unit.id}`)}>
              <CardContent className="p-4 space-y-3">
                ...
                <p className="font-mono text-xs text-muted-foreground">{unit.vin}</p>
                <div className="flex items-center justify-between text-sm">
                  ...
                  <div className="flex items-center gap-1 font-semibold"><CurrencyDollar className="h-3.5 w-3.5" />${unit.askingPrice.toLocaleString()}</div>
                </div>
              </CardContent>
            </Card>
          )
